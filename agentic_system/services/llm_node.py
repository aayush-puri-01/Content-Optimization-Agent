...
def llm_router(state: CampaignState) -> dict:
    ...
    if not state.current_step:
        ...
        try:
            ...
            steps = [
                {"step": s, "executed": False}
                for s in result.get("steps", ["trend_analyzer"])
                if s in ["trend_analyzer", "search_engine", "hashtag_generator", "script_generator"]
            ]
            if not params.get("campaign_theme"):
                return {
                    "messages": state.messages + [{"role": "assistant", "content": "Please provide a campaign theme."}],
                    "current_step": "END"
                }
            state_updates = {
                ...
                "steps": steps,
                "messages": state.messages + [{"role": "system", "content": f"Parameters: {params}, Steps: {steps}"}],
            ...
            return {
                "messages": state.messages + [{"role": "assistant", "content": f"Error initializing workflow: {e}"}],
                "current_step": "END"
            }
    ...
    prompt = (
        ...
        "Available tools: trend_analyzer, search_engine, hashtag_generator, script_generator.\n"
        "Return a JSON object with 'current_step' (tool name or 'END') and 'new_steps' (list of new steps, if any)."
    )
    try:
        response = llm.invoke([{"role": "user", "content": prompt}], response_format={"type": "json_object"})
        result = json.loads(response.content)
        ...
        return {
            "steps": steps,
            "current_step": next_step if next_step in ["trend_analyzer", "search_engine", "hashtag_generator", "script_generator", "END"] else "END"
        }
    except Exception as e:
        print(f"Error in dynamic routing: {e}")
        return {
            "steps": steps,
            "messages": state.messages + [{"role": "assistant", "content": f"Error determining next step: {e}"}],
            "current_step": "END"
        }